...
sub CheckTables{
	...
	my $total;
	my $sql = "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'spampot';";
	...
	BASE::logMsgT($fcaller,"Checking the total of tables on DB: $total\n",2,$GLOBAL_VARS::LOG_FH);
	return $total;
}
...
sub CreateTables{
	...
	BASE::logMsgT($fcaller,"Created table Binaries",2,$GLOBAL_VARS::LOG_FH);
	##################################################################
	...
	BASE::logMsgT($fcaller,"Created table IPs",2,$GLOBAL_VARS::LOG_FH);   
	##################################################################
	...
	BASE::logMsgT($fcaller,"Created table Subjects",2,$GLOBAL_VARS::LOG_FH);    
	##################################################################
	...
	BASE::logMsgT($fcaller,"Created table Urls",2,$GLOBAL_VARS::LOG_FH);
	##################################################################
	...
	BASE::logMsgT($fcaller,"Created table Usernames",2,$GLOBAL_VARS::LOG_FH);
}
1;